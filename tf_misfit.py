...
def plot_tf_misfits(st1, st2, dt=0.01, t0=0., fmin=1., fmax=10., nf=100, w0=6,
                    ...
                    clim=0., cmap=obspy_divergent, label=['signal 1', 'signal 2'],
    ...
    for itr in np.arange(ntr):
        ...
        ax_sig.text(-0.2, 0.5, textstr, transform=ax_sig.transAxes,
        ...
        if title:
            ...
            fig.suptitle("Time-Frequency misfit between signal 1 and signal 2")
...
def plot_tf_gofs(st1, st2, dt=0.01, t0=0., fmin=1., fmax=10., nf=100, w0=6,
                 ...
                 bottom=0.25, h_1=0.2, h_2=0.125, h_3=0.2, w_1=0.2, w_2=0.6,
    ...
    for itr in np.arange(ntr):
        ...
        print ('Plotting', itr+1)

        # plot signals
        ax_sig = fig.add_axes([left + w_1, bottom + h_2*1.1 + h_3, w_2, h_1*0.75])
        # ax_sig.plot(t, st2[itr], plot_args[1], color='black',label='Reference' )
        # ax_sig.plot(t, st1[itr], plot_args[0], color='red',  label='SEM')


        # 1D vs 2D SEM
        ax_sig.plot(t, st2[itr], plot_args[1], color='black',label='1D' )
        ax_sig.plot(t, st1[itr], plot_args[0], color='red',  label='2D')

        # # Moindze 1
        # ax_sig.plot(t, st2[itr], plot_args[1], color='black',label='Implicit-Implicit' )
        # ax_sig.plot(t, st1[itr], plot_args[0], color='red',  label='Implicit-Explicit')

        # # Moindze 2
        # ax_sig.plot(t, st2[itr], plot_args[1], color='black',label='Explicit-Explicit' )
        # ax_sig.plot(t, st1[itr], plot_args[0], color='red',  label='Implicit-Explicit')



        # Legend box for signal time histories
        plt.legend(bbox_to_anchor=(-0.11, 0.5),fancybox=True, prop={'size':7.0})
        plt.tick_params(axis='both', which='major', labelsize=10)


        # plot TEG
        ax_teg = fig.add_axes([left + w_1, bottom + h_1*0.85 + h_2*1.1 + h_3, w_2, h_2*0.85])
        ...
        # plot TPG
        ax_tpg = fig.add_axes([left + w_1, bottom*0.7, w_2, h_2])
        ...
        # ROME MODEL - PAPER 1
        print ('Attention!')
        print ('Changed clim for Rome model')
        img_tfpg.set_clim(9.9, clim)
        img_tfeg.set_clim(9.9, clim)


        # MOINDZE MODEL - PAPER 1
        # print ('Attention!')
        # print ('Changed clim for Moindze')
        # img_tfpg.set_clim(7.9, clim)
        # img_tfeg.set_clim(7.9, clim)
        ...
        ax_sig.text(-0.3, 1.3, textstr, transform=ax_sig.transAxes,
                    verticalalignment='center', horizontalalignment='left',
                    bbox=props)



        ax_tpg.set_xlabel('Time [s]')
        ax_feg.set_ylabel('Frequency [Hz]')
        ax_fpg.set_ylabel('Frequency [Hz]')
